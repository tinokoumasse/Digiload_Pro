...
def init_db():
    with sqlite3.connect(_DB_FILE) as conn:
        ...
        -- Scan queue: bridge between agent.py (HTTP receiver) and digiload_pro.py (state machine)
        ...
        CREATE INDEX IF NOT EXISTS idx_scan_queue_pending
            ON scan_queue(processed) WHERE processed = 0;
        """)
...
# ─────────────────────────────────────────────────────────────────────────────
# HID SCANNER LISTENER (channel 3) — captures USB/Bluetooth scanner directly
# Works in background regardless of OpenCV window focus
# ─────────────────────────────────────────────────────────────────────────────
_HID_KEYMAP = {
...
def hid_scanner_listener():
    """
    Auto-detect USB/Bluetooth HID barcode scanners and read scans directly.
    Most barcode scanners present as HID keyboards. We grab the device
    exclusively so the scans don't leak into other apps.
    """
    try:
        import evdev
    except ImportError:
        log.warning("[hid] evdev not installed — channel 3 (ZED Box scanner) disabled")
        log.warning("[hid] Install with: pip install evdev")
        return
        ...
        try:
            ...
            for path in evdev.list_devices():
                try:
                    ...
                    # Heuristic: barcode scanners usually have "scanner", "barcode",
                    # "honeywell", "zebra", "datalogic", "newland" in their name
                    is_scanner = any(kw in name_lower for kw in
                        ["scan", "barcode", "honeywell", "zebra", "datalogic",
                         "newland", "symbol", "hid"])
                    # Skip obvious non-scanners
                    is_not_scanner = any(kw in name_lower for kw in
                        ["mouse", "keyboard ", "touchpad", "trackpad"])
                    if is_scanner and not is_not_scanner:
                        scanners.append(dev)
                        log.info(f"[hid] Found scanner: {dev.name} ({path})")
                except Exception:
                    continue

            if not scanners:
                log.debug("[hid] No HID scanner detected — retrying in 30s")
                time.sleep(30)
                continue

            # Listen on first scanner found
            dev = scanners[0]
            try:
                dev.grab()  # exclusive access — prevents OS from also receiving
                log.info(f"[hid] Listening on {dev.name}")
            except Exception as e:
                log.warning(f"[hid] Cannot grab exclusively (may need sudo): {e}")

            buf = ""
            for event in dev.read_loop():
                if event.type != evdev.ecodes.EV_KEY or event.value != 1:
                    continue   # only key-down events
                try:
                    ...
                    if key == "KEY_ENTER":
                        if buf:
                            handle_sscc_scan(buf, source="hid")
                            buf = ""
                    elif key in _HID_KEYMAP:
                        buf += _HID_KEYMAP[key]
                    # Silently ignore other keys (modifiers, function keys)
                except Exception:
                    continue
        except OSError as e:
            log.warning(f"[hid] Scanner disconnected: {e} — re-scanning in 5s")
            time.sleep(5)
        except Exception as e:
            log.error(f"[hid] Listener error: {e}")
            time.sleep(10)


def poll_scan_queue():
    """Poll scan_queue table for scans pushed by agent.py (channel 1, 2)."""
    while True:
        try:
            with get_db() as conn:
                rows = conn.execute(
                    "SELECT id, sscc, source FROM scan_queue WHERE processed=0 ORDER BY id LIMIT 5"
                ).fetchall()
            for row in rows:
                result = handle_sscc_scan(row["sscc"], source=row["source"] or "json")
                with get_db() as conn:
                    conn.execute(
                        "UPDATE scan_queue SET processed=1, result=?, processed_at=datetime('now') WHERE id=?",
                        (json.dumps(result), row["id"])
                    )
        except Exception as e:
            log.debug(f"[scan_queue] {e}")
        time.sleep(0.2)
...
# SSCC scan deduplication — track last scan to reject duplicates within window
_SCAN_DEDUP_WINDOW_S = 2.0
_last_scan_sscc = None
_last_scan_time = 0.0
_last_scan_lock = threading.Lock()

def handle_sscc_scan(sscc, source="hid"):
    """
    Process an SSCC scan from any channel (hid, tablet, json).
    Returns: dict with {accepted, reason, mode, message}
    """
    sscc = (sscc or "").strip()
    if not sscc:
        return {"accepted": False, "reason": "empty", "message": "Empty SSCC"}

    # ── Dedup check ───────────────────────────────────────────────────────
    global _last_scan_sscc, _last_scan_time
    now = time.time()
    with _last_scan_lock:
        if _last_scan_sscc == sscc and (now - _last_scan_time) < _SCAN_DEDUP_WINDOW_S:
            log.warning(f"[scan] DUPLICATE rejected: {sscc} (source={source})")
            st.last_scan_display = f"ALREADY SCANNED: {sscc}"
            return {
                "accepted": False,
                "reason":   "duplicate",
        ...
        _last_scan_sscc = sscc
        _last_scan_time = now

    log.info(f"[scan] {source} → {sscc}")

    if st.app_mode != "STANDBY" or not st.active_tour_id:
        st.last_scan_display = f"OUT OF SESSION: {sscc}"
        log.warning(f"[scan] OOS: {sscc}")
        return {
            "accepted": False, "reason": "out_of_session",
            "message":  "No active mission",
            "sscc":     sscc, "mode": st.app_mode,
        }

    if db_check_sscc(sscc, st.active_tour_id):
        transition("ARMED", sscc=sscc)
        st.last_scan_display = f"OK: {sscc}"
        return {
            "accepted": True, "reason": "valid",
            "message":  "Drive through the gate",
            "sscc":     sscc, "mode": "ARMED",
        }
    else:
        db_flag_sscc(sscc, st.active_tour_id)
        transition("ERROR_SSCC")
        st.last_scan_display = f"INVALID: {sscc}"
        log.warning(f"[scan] Invalid: {sscc}")
        if st.plugins:
            st.plugins.on_error("WRONG_SSCC", {
                "gate_id": st.gate_id, "sscc": sscc, "tour_id": st.active_tour_id
            })
        return {
            "accepted": False, "reason": "invalid_sscc",
            "message":  "SSCC not in mission",
            "sscc":     sscc, "mode": "ERROR_SSCC",
        }
...
def run():
    ...
    threading.Thread(target=hid_scanner_listener, daemon=True).start()
