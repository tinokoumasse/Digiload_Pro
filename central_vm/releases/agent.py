...
# ─────────────────────────────────────────────────────────────────────────────
# SSCC SCAN ENDPOINT (channels 1 + 2) — JSON API and tablet HID forwarding
# ─────────────────────────────────────────────────────────────────────────────
@app.route("/scan", methods=["POST"])
@app.route("/agent/scan", methods=["POST"])
def receive_scan():
    """
    Receive an SSCC scan from any source and queue it for digiload_pro.py.
    Channels:
      - JSON API:        external systems POST {"sscc": "..."}
      - Tablet HID:      driver tablet scanner forwards here
      - Tablet camera:   driver tablet camera scan forwards here
      - Dashboard HID:   VM forwards scanner input here

    Returns the scan result after digiload_pro.py processes it (~300ms).
    """
    data   = request.get_json(silent=True) or {}
    sscc   = (data.get("sscc")   or "").strip()
    source = (data.get("source") or "json").strip()

    if not sscc:
        return jsonify({"ok": False, "error": "sscc field required"}), 400

    # Insert into scan_queue — digiload_pro.py picks it up within 200ms
    try:
        conn = sqlite3.connect(_DB_FILE, timeout=5)
        cur  = conn.execute(
            "INSERT INTO scan_queue (sscc, source) VALUES (?, ?)",
            (sscc, source)
        )
        ...
        log.error(f"[scan] DB insert failed: {e}")
        return jsonify({"ok": False, "error": "queue insert failed"}), 500

    log.info(f"[scan] Received: {sscc} (source={source}) → queue id {scan_id}")

    # Wait briefly for digiload_pro.py to process it (so we can return result)
    for _ in range(15):   # up to 1.5 seconds
        time.sleep(0.1)
        try:
            conn = sqlite3.connect(_DB_FILE, timeout=2)
            row  = conn.execute(
                "SELECT processed, result FROM scan_queue WHERE id=?",
                (scan_id,)
            ).fetchone()
            conn.close()
            if row and row[0] == 1:
                import json as _json
                result = _json.loads(row[1]) if row[1] else {}
                return jsonify({"ok": True, **result})
        except Exception:
            continue

    # Timeout — scan was queued but digiload_pro.py didn't respond
    return jsonify({
        "ok":      True,
        "queued":  True,
        "message": "Scan queued (no immediate response)",
        "sscc":    sscc,
    })
